...
\label{subsec:project_background}
...
% ใส่ต่อจาก \subsection{ขอบเขตของโครงงาน} 
% และก่อน \subsection{ภาพรวมระบบ}
...
โครงงานนี้มีขอบเขตที่ชัดเจนโดยไม่ครอบคลุมส่วนประกอบ
ที่อยู่ภายใต้ความรับผิดชอบของลูกค้า ได้แก่

\begin{itemize}
  \item \textbf{Base System ของลูกค้า}:
        ซอฟต์แวร์และฮาร์ดแวร์ของระบบควบคุมหลัก
        ที่ส่งคำสั่งผ่าน Modbus ไม่ได้อยู่ในขอบเขต
        การออกแบบและทดสอบของโครงงานนี้
        โครงงานรับประกันเฉพาะด้าน Interface
        ของ Modbus RTU ฝั่ง Slave เท่านั้น

  \item \textbf{End Effector (Gripper)}:
        ชุดหัวจับที่ลูกค้ากำหนดไม่ได้อยู่ในขอบเขต
        การออกแบบ โครงงานนี้รับผิดชอบเฉพาะ
        End Effector Housing และสัญญาณควบคุม
        Solenoid Valve สำหรับ Gripper เท่านั้น

  \item \textbf{ระบบสายพานลำเลียง}:
        สายพานลำเลียงชิ้นงานต้นทางและปลายทาง
        ไม่ได้อยู่ในขอบเขต โครงงานสมมติว่า
        Connection Rod อยู่ที่ตำแหน่ง Pick/Place
        ที่กำหนดไว้แล้วก่อนสั่งงาน

  \item \textbf{การควบคุมแกนแนวดิ่ง (Z-axis)}:
        ระบบนี้เป็นหุ่นยนต์ 1 แกน (Rotary)
        การเคลื่อนที่แนวดิ่งของ Gripper
        ดำเนินการโดย Pneumatic Actuator
        ของลูกค้าโดยตรง ไม่ใช่โดย Controller
        ของโครงงานนี้

  \item \textbf{การทดสอบอายุการใช้งานระยะยาว}:
        ข้อกำหนด M5 กำหนดอายุใช้งาน
        $\geq 3$ รอบทดสอบ (รอบละ 1 ชั่วโมง)
        การทดสอบ Fatigue และ Wear
        ในระยะยาวเกินขอบเขตของโครงงานนี้
\end{itemize}

% ------------------------------------------------------------
\subsection{ภาพรวมโครงสร้างรายงาน (Report Organization)}
\label{subsec:report_org}
% ------------------------------------------------------------

รายงานฉบับนี้จัดแบ่งเนื้อหาออกเป็น 7 บทหลัก เพื่อให้ครอบคลุมการดำเนินงานตั้งแต่การออกแบบจนถึงการทวนสอบระบบ ดังนี้

\begin{enumerate}
    \item \textbf{บทที่ 1 บทนำ (Introduction):} อธิบายที่มาและความสำคัญ วัตถุประสงค์ ขอบเขตของโครงงาน และข้อกำหนดของระบบ (System Requirements) ทั้ง 32 ข้อ
    \item \textbf{บทที่ 2 การออกแบบระบบ (System Design):} นำเสนอรายละเอียดการออกแบบเชิงกล (Mechanical Design) ระบบไฟฟ้า (Electrical Design) และระบบควบคุม (Control System Design) รวมถึงการวิเคราะห์ผลกระทบของ Connection Rod ต่อการคำนวณ Cycle Time
    \item \textbf{บทที่ 3 การดำเนินการสร้างและติดตั้งระบบ (Implementation):} บันทึกขั้นตอนการประกอบจริงของโครงสร้างเชิงกล การจัดทำตู้ควบคุมไฟฟ้า และโครงสร้างสถาปัตยกรรม Firmware
    \item \textbf{บทที่ 4 การบูรณาการระบบ (System Integration):} อธิบายการเชื่อมต่อระบบย่อยทั้งหมดเข้าด้วยกัน รายละเอียด Modbus Register Map และแผนการทดสอบระบบแบบ End-to-End
    \item \textbf{บทที่ 5 ผลการทดสอบ (Test Results):} รวบรวมและรายงานผลการทดสอบเชิงประจักษ์ โดยแบ่งตามระยะการดำเนินงานใน Progress Update 1 และ Progress Update 2
    \item \textbf{บทที่ 6 การตรวจสอบและทวนสอบ (Verification and Validation):} นำเสนอกรอบการ V\&V ที่ครอบคลุมข้อกำหนดของระบบทั้ง 32 Requirements เพื่อยืนยันความถูกต้องของคุณลักษณะหุ่นยนต์
    \item \textbf{บทที่ 7 สรุปผลและแนวทางต่อไป (Conclusion and Future Work):} สรุปภาพรวมความสำเร็จของโครงงาน ปัญหาที่พบ และแผนการดำเนินงานต่อเนื่องก่อนถึงการนำเสนอ Final Demo
\end{enumerate}
...
ระบบ Circular Pick and Place Robot ออกแบบโดยแบ่ง
สถาปัตยกรรมออกเป็น 2 ระดับ ได้แก่
Physical Architecture ซึ่งอธิบายฮาร์ดแวร์จริงของระบบ
และ Cyber Architecture ซึ่งอธิบายโครงสร้างซอฟต์แวร์
ภายใน STM32G474RE ดังแสดงในรูปที่~\ref{fig:sys_arch}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{images/system_architecture.png}
  \caption{System Architecture ของระบบ
           Circular Pick and Place Robot
           แสดง Physical Architecture
           และ Cyber Architecture}
  \label{fig:sys_arch}
\end{figure}

ระบบแบ่งออกเป็น 4 Subsystem หลักที่ทำงานร่วมกัน
ดังนี้

\paragraph{Rotating Mechanism}
เป็นส่วนที่แปลงพลังงานไฟฟ้าจาก DC Motor
ให้กลายเป็นการเคลื่อนที่เชิงมุมของแขนกล
กำลังถูกส่งผ่าน Planetary Gearbox (อัตราทด 24)
ต่อด้วย Belt Drive HTD5M (อัตราทด 4.5)
รวมอัตราทดทั้งหมด 70 ก่อนถึงแขนกลรัศมี 500\,mm
ตำแหน่งเชิงมุมวัดโดย Encoder AMT-103
ที่ติดตั้งโดยตรงที่ Output Shaft
...
ที่แยกจากกันอย่างชัดเจน
High Power Bus (24\,V/10\,A) จ่ายให้มอเตอร์
ผ่าน Magnetic Contactor ที่ถูกควบคุมโดย E-Stop
และ Logic Bus (5\,V/5\,A) จ่ายให้ STM32,
ESP32 และ Sensor โดยต่อก่อน Contactor
ทำให้ Controller ยังมีไฟเลี้ยงแม้กด E-Stop
ตามข้อกำหนด E6

\paragraph{Main Controller}
STM32G474RE ทำหน้าที่เป็นศูนย์กลางประมวลผล
ทั้งหมด รัน Cascade PID Control Loop
ที่ความถี่ 100\,Hz ประกอบด้วย Velocity Inner Loop
และ Position Outer Loop
พร้อม S-Curve Trajectory Generator
ที่จำกัด Jerk เพื่อลด Mechanical Shock
และรับส่งข้อมูลกับ Base System
ผ่าน Modbus RTU บน LPUART1

\paragraph{External Interface}
รองรับการสั่งงาน 2 โหมด ได้แก่
Manual Mode ผ่าน Wireless Joystick
ที่รับสัญญาณ Bluetooth ด้วย ESP32
และส่งต่อผ่าน USART3 มายัง STM32
และ Auto Mode ผ่าน Base System ของลูกค้า
โดยรับคำสั่งผ่าน Modbus RTU
(Slave Address 21, 19200\,bps, 8E1)
...
\begin{table}[H]
  ...
  \begin{tabular}{lp{4.2cm}p{4cm}}
    ...
    \midrule
    Rotating Mechanism
      & DC Motor, Planetary Gearbox ($N_g$=24),
        Belt HTD5M ($N_b$=4.5),
        Azimuth Joint, Encoder AMT-103
      & Section~\ref{subsec:mechanical_design} \\
    Power Management
      & PSU 24V/10A, PSU 5V/5A,
        Magnetic Contactor, E-Stop,
        Circuit Breaker, Fuse
      & Section~\ref{subsec:electronics_design} \\
    Main Controller
      & STM32G474RE, Cascade PID (100\,Hz),
        S-Curve Trajectory,
        Modbus RTU Slave
      & Section~\ref{subsec:control_design},
        Section~\ref{subsec:firmware_architecture} \\
    External Interface
      & Joystick + ESP32 (Bluetooth),
        Base System (Modbus RTU,
        Slave Address 21)
      & Section~\ref{subsec:joystick_connection},
        Section~\ref{subsec:modbus_connection} \\
    \bottomrule
  \end{tabular}
\end{table}

รายละเอียดการออกแบบของแต่ละ Subsystem
แสดงในบทที่~\ref{sec:system_design}
การ Implement จริงในบทที่~\ref{sec:implementation}
และการบูรณาการระหว่าง Subsystem
ในบทที่~4
