...
\label{subsubsec:mechanical_architecture}
...
\label{subsec:deflection}
% ------------------------------------------------------------

\subsubsection{การวิเคราะห์โครงสร้างแขนกลด้วย FEA}
\label{subsubsec:arm_fea_analysis}

การวิเคราะห์ Finite Element Analysis (FEA)
ดำเนินการด้วย SOLIDWORKS Simulation (Static Study)
เพื่อตรวจสอบความแข็งแรงและการโก่งตัวของแขนกล
ภายใต้โหลดสูงสุด 2\,kg ตามข้อกำหนด M3

\paragraph{คุณสมบัติวัสดุ}
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    Tensile Strength  & $4.00 \times 10^{8}$     & N/m$^2$ \\
    Elastic Modulus   & $2.00 \times 10^{11}$    & N/m$^2$ \\
    Poisson's Ratio   & 0.26                     & -- \\
    Mass Density      & 7,850                    & kg/m$^3$ \\
    Shear Modulus     & $7.93 \times 10^{10}$    & N/m$^2$ \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{เงื่อนไขการวิเคราะห์}

\begin{itemize}
  \item \textbf{Fixture}: Fixed Geometry ที่ปลายด้านยึด
        (Fixed-4: 1 edge, 7 faces;
         Fixed-5: 5 faces)
  \item \textbf{Load}: Gravity $-9.81$\,m/s$^2$
        และ Remote Load (Distributed) มวล 2.76\,kg
        ที่ตำแหน่งอ้างอิง
        $(-0.9,\ -46.82,\ 55.88)$\,mm
        จำลอง Gripper และ Connection Rod รวมกัน
  \item \textbf{Mesh}: Solid Mesh,
        Blended Curvature-based,
        Element Size 1.15--11.82\,mm,
        High Quality (Jacobian 16 Points),
        11,607 Nodes, 5,722 Elements,
        Max Aspect Ratio 8.181
\end{itemize}

\paragraph{ผลการวิเคราะห์ FEA}

\begin{table}[H]
  \centering
  \caption{ผลการวิเคราะห์ FEA เทียบกับเกณฑ์ M3}
  \label{tab:fea_results}
  \begin{tabular}{lcccc}
    \toprule
    พารามิเตอร์ & ผล FEA & เกณฑ์ M3 & หน่วย & ผล \\
    \midrule
    Max Displacement (URES)
      & 0.2281 & $\leq 1.0$ & mm & \textbf{ผ่าน} \\
    Max von Mises Stress
      & $2.131 \times 10^7$ & $< 2.35 \times 10^8$ & N/m$^2$ & \textbf{ผ่าน} \\
    Min Factor of Safety
      & 11.03 & $> 1.0$ & -- & \textbf{ผ่าน} \\
    \bottomrule
  \end{tabular}
\end{table}

ผลการวิเคราะห์แสดงให้เห็นว่าการโก่งตัวสูงสุด
$\delta_{max} = 0.2281$\,mm ต่ำกว่าเกณฑ์ 1\,mm
อย่างมีนัยสำคัญ และค่าความเค้น Von Mises สูงสุด
$\sigma_{max} = 2.131 \times 10^7$\,N/m$^2$
คิดเป็นเพียง 9.1\% ของ Yield Strength
($\sigma_y = 2.35 \times 10^8$\,N/m$^2$)
ส่งผลให้ Factor of Safety ต่ำสุด = 11.03
ซึ่งสูงกว่าเกณฑ์ขั้นต่ำอย่างปลอดภัย
โครงสร้างแขนกลจึงรับโหลด 2\,kg
ได้อย่างปลอดภัยตามข้อกำหนด M3

% [รูป Von Mises Stress]
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.82\textwidth]{images/fea_stress.png}
%   \caption{ผล FEA: Von Mises Stress Distribution
%            บนแขนกลภายใต้โหลด 2\,kg
%            (Max = $2.131 \times 10^7$\,N/m$^2$,
%             Yield Strength = $2.35 \times 10^8$\,N/m$^2$)}
...
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.82\textwidth]{images/fea_displacement.png}
%   \caption{ผล FEA: Resultant Displacement
%            บนแขนกลภายใต้โหลด 2\,kg
%            (Max = 0.2281\,mm)}
%   \label{fig:fea_displacement}
% \end{figure}

% [รูป Factor of Safety]
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.82\textwidth]{images/fea_fos.png}
%   \caption{ผล FEA: Factor of Safety Distribution
%            (Min FOS = 11.03 ที่จุดยึดแขนกล)}
%   \label{fig:fea_fos}
% \end{figure}
...
% ------------------------------------------------------------
\subsection{การคำนวณกำลังไฟฟ้า (Power Calculation)}
\label{subsec:power_calc}
% ------------------------------------------------------------

การเลือก Power Supply ดำเนินการโดยคำนวณกระแสรวม
สูงสุดของอุปกรณ์ทุกตัวในแต่ละ Bus พร้อม Safety Margin
20\% แบ่งออกเป็น 2 วงจรหลัก

\paragraph{24V Motor Bus}

\begin{table}[H]
  \centering
  \caption{การคำนวณกำลังไฟฟ้าของ 24V Motor Bus (ค่าอัปเดต)}
  \label{tab:power_24v}
  \begin{tabular}{lccrp{4.5cm}}
    \toprule
    อุปกรณ์ & แรงดัน (V) & กระแสสูงสุด (A) & กำลังไฟฟ้า (W) & หมายเหตุ \\
    \midrule
    DC Motor ZGX60RMM  & 24 & 10.00 & 240.00 &
      Peak current จาก Simulation
      (S-Curve 360°, ตารางที่~\ref{tab:sim_vs_req}) \\
    ...
    \midrule
    \textbf{รวม 24V Bus}
      & \textbf{24} & \textbf{13.08} & \textbf{313.92} & \\
    \bottomrule
  \end{tabular}
\end{table}

ผลการคำนวณเมื่อรวม Safety Margin 20\%:
กระแสรวม 13.08\,A $>$ 10\,A ของ PSU 24V/10A
อย่างไรก็ตาม Peak Current 10\,A เกิดขึ้นเพียงชั่วขณะ
(Current Clamp Zone = 0.71\% ของเวลาการเคลื่อนที่ทั้งหมด
ดังแสดงในตารางที่~\ref{tab:sim_vs_req})
ซึ่งในการทดสอบ Hardware จริงไม่พบปัญหา
PSU Voltage Drop หรือ Overcurrent Trip

เพื่อให้ระบบมีความปลอดภัยในระยะยาว
จะ Implement Software Current Clamp
ใน Firmware ก่อน Demo โดยจำกัด
Duty Cycle สูงสุดไม่ให้กระแสมอเตอร์
เกิน 8\,A (80\% ของพิกัด PSU)
ตามสมการ
...
\begin{table}[H]
  \centering
  \caption{การคำนวณกำลังไฟฟ้าของ 5V Logic Bus (ค่าอัปเดต)}
  \label{tab:power_5v}
  \begin{tabular}{lccr}
    \toprule
    ...
    \midrule
    \textbf{รวม 5V Bus}
      & \textbf{5} & \textbf{1.63} & \textbf{8.16} \\
    \bottomrule
  \end{tabular}
\end{table}

ผลการคำนวณ: กระแสรวม 1.63\,A $<$ 5\,A ของ
PSU 5V/5A ที่เลือกใช้ \textbf{(ผ่าน)}

\paragraph{5V Logic Bus}

\begin{table}[H]
  \centering
  \caption{การคำนวณกำลังไฟฟ้าของ 5V Logic Bus}
  \label{tab:power_5v}
  \begin{tabular}{lccr}
    \toprule
    อุปกรณ์ & แรงดัน (V) & กระแสสูงสุด (A) & กำลังไฟฟ้า (W) \\
...
ผลการคำนวณ: กระแสรวม 1.63\,A $<$ 4\,A ของ
PSU 5V/4A ที่เลือกใช้ \textbf{(ผ่าน)}
...
\begin{table}[H]
  \centering
  \caption{การเลือกขนาด Fuse}
  \label{tab:fuse_select}
  \begin{tabular}{lccl}
    \toprule
    วงจร & กระแสโหลด (A) & ขนาดขั้นต่ำ ($\geq$1.25$\times$load) & ขนาดที่เลือก \\
    \midrule
    DC 24V -- Motor Branch & 5.28 & 6.60\,A & 8\,A \\
    DC 5V -- Logic Branch  & 1.63 & 2.04\,A & 2.5\,A \\
    \bottomrule
  \end{tabular}
\end{table}
